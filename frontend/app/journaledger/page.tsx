...
export default function JournalLedger() {
    ...
    useEffect(() => {
        if (!user || !selectedSector) { setStocks([]); return }
            ...
            if (data) {
                const filtered = data
                    ...
                    .filter((s: StockForJL) =>
                        ...
                        (s.interest === 'interested' || s.is_portfolio)
                    )
                setStocks(filtered)
            }
        }
        fetchStocks()
    }, [user, selectedSector, supabase])
    ...
    const fetchTransactions = async (mode: 'overall' | 'single' | 'sector', companyName?: string, sectorId?: string) => {
        ...
        try {
            ...
            if (mode === 'single' && companyName) {
                // Get stock_id by company name
                ...
                // Get all stock_ids in this sector
            ...
            <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
                ...
                {!selectedSector ? (
                    <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-white/10 py-20 text-center">
                        ...
                        <h3 className="text-lg font-medium text-white">Select a Sector</h3>
                        <p className="mt-1 text-sm text-gray-400">Choose a sector from the dropdown to view your interested & portfolio companies.</p>
                    </div>
                ) : stocks.length === 0 ? (
                    <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-white/10 py-20 text-center">
                        <div className="mb-4 rounded-full bg-white/5 p-4">
                        ...
                        <p className="mt-1 text-sm text-gray-400">No interested or portfolio companies in this sector.</p>
                    </div>
                ) : (
                            ...
                            <div key={stock.alert_id} className="relative overflow-hidden rounded-xl border border-blue-500/20 bg-white/5 p-5 backdrop-blur-lg transition-all hover:border-blue-500/30 hover:bg-white/10">
                                ...
                                <div className="mb-3">
                                    <div className="flex items-center gap-2 mb-0.5">
                                        ...
                                        {stock.interest === 'interested' && !stock.is_portfolio && (
                                            <span className="rounded-full bg-blue-500/15 px-2 py-0.5 text-xs font-medium text-blue-300">Interested</span>
                                        )}
                                    </div>
                                ...
                                <div className="flex gap-2 border-t border-white/5 pt-3">
                                    <button
                                        ...
                                        className="flex-1 flex items-center justify-center gap-1.5 rounded-lg bg-green-600/15 px-3 py-2.5 text-sm font-semibold text-green-400 hover:bg-green-600/25 transition-colors"
                                    >
                                    ...
                                    <button
                                        ...
                                        className="flex-1 flex items-center justify-center gap-1.5 rounded-lg bg-red-600/15 px-3 py-2.5 text-sm font-semibold text-red-400 hover:bg-red-600/25 transition-colors"
                                    >
                    ...
                    <div className="w-full max-w-md rounded-2xl border border-amber-500/30 bg-[#0a0a0f] p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
                        ...
                        {trackMode === 'single' && (
                            <div className="mb-4">
                                ...
                                <input type="text" value={trackCompanyName} onChange={e => setTrackCompanyName(e.target.value)} className="w-full rounded-md border border-white/10 bg-white/5 px-3 py-2 text-white focus:border-amber-500 focus:outline-none" placeholder="Enter exact company name" />
                            </div>
                        )}
                        {trackMode === 'sector' && (
                            <div className="mb-4">
